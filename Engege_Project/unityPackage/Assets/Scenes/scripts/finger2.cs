...
public class finger2 : MonoBehaviour
{
    ...
    public void catchRotation(float rotateAngle)
    {
        if (accumulatedRotation < targetRotation)
        {
           ...
           // Debug.Log("Finger 2 Current rotation is "+currentRotation + "after rotate for "+rotationThisFrame);
        }
    }

     public void catchRotationBack(float rotateAngle)
{
    ...
    if (accumulatedRotation < 0)
    {
        ...
        if (rotateAngle > -accumulatedRotation)
        {
            rotateAngle = accumulatedRotation;
        }

        // 减少累积的旋转角度
        accumulatedRotation += rotateAngle;

        // 获取当前的欧拉角
        Vector3 currentRotation = transform.rotation.eulerAngles;

        // 往回旋转，恢复到之前的角度
        currentRotation.z += rotateAngle;

        // 设置新的旋转
        transform.rotation = Quaternion.Euler(currentRotation);

       // Debug.Log("catchRotationBack method called");
       // Debug.Log("Finger 2 Current rotation is " + currentRotation + " after rotating back for " + rotateAngle);
    }
}
